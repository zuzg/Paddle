...
class TestConv2DTransposeInt8MKLDNNOp(TestConv2DTransposeOp):
    ...
    def setUp(self):
        ...
        def conv2d_forward_refer_helper(input_):
            return conv2dtranspose_forward_naive(input_.astype(np.int32), filter_int, self.attrs
                                                ).astype(np.float32) * scale_output_shift
    ...
    def init_test_case(self):
        TestConv2DTransposeOp.init_test_case(self)
...
class TestWithRelu6(TestConv2DTransposeInt8MKLDNNOp):
...
class TestWithSwish(TestConv2DTransposeInt8MKLDNNOp):
...
class TestWithPad(TestConv2DTransposeInt8MKLDNNOp):

    def init_test_case(self):
        super(TestWithPad, self).init_test_case()
        self.pad = [1, 1]
        self.input_size = [2, 3, 10, 10]


class TestWithGroup(TestConv2DTransposeInt8MKLDNNOp):

    def init_test_case(self):
        super(TestWithGroup, self).init_test_case()
        self.groups = 3


class TestWithStride(TestConv2DTransposeInt8MKLDNNOp):

    def init_test_case(self):
        super(TestWithStride, self).init_test_case()
        self.pad = [1, 1]
        self.stride = [2, 2]
...
class TestWithDilations(TestConv2DTransposeInt8MKLDNNOp):

    def init_test_case(self):
        super(TestWithDilations, self).init_test_case()
        self.pad = [1, 1]
        self.stride = [1, 1]
        self.dilations = [2, 2]
        self.input_size = [2, 3, 10, 10]
...
def init_data_type_with_fusion(self, input_dt, fuse_activation):
...
create_test_int8_class(TestConv2DTransposeInt8MKLDNNOp)
...
class TestConv2DOp_AsyPadding_INT_MKLDNN(TestConv2DTransposeInt8MKLDNNOp):
